...
printf '%s' "$AUTH_QR_OUTPUT" | node -e 'const fs=require("fs");let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const action=json.checkout && json.checkout.human_action; if (json.status !== "checkout_created" || !action || action.kind !== "auth_qr") process.exit(1); if (!action.local_qr_path || !fs.existsSync(action.local_qr_path)) process.exit(1); if (action.local_qr_mime !== "image/png") process.exit(1); if (action.agent_display_hint.primary !== "local_qr_path") process.exit(1); if (!json.render_plan || json.render_plan.kind !== "auth_qr" || !json.render_plan.platforms.telegram.buttons.length) process.exit(1); if (action.qr_png_url || action.qr_image_url || action.preferred_qr_url) process.exit(1);})'
...
printf '%s' "$BUY_CODEX_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); if (json.status !== "payment_handoff_required" || json.must_reply_to_human_before_next_command !== true || !json.human_visible_markdown.includes("![ItPay payment QR](") || json.next.safe_for_agent !== false) process.exit(1); if (!json.render_plan || json.render_plan.host !== "codex" || !json.render_plan.platforms.codex_app.markdown.includes("mock-qr.png")) process.exit(1);})'
...
printf '%s' "$PAYMENT_WAIT_HANDOFF_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); if (json.status !== "payment_handoff_required" || json.next.safe_for_agent !== false || !json.render_plan) process.exit(1);})'
