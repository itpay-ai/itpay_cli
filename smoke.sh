...
printf '%s' "$AUTH_QR_OUTPUT" | node -e 'const fs=require("fs");let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const action=json.checkout && json.checkout.human_action; if (json.status !== "checkout_created" || !action || action.kind !== "auth_qr") process.exit(1); if (!action.local_qr_path || !fs.existsSync(action.local_qr_path)) process.exit(1); if (action.local_qr_mime !== "image/png") process.exit(1); if (action.agent_display_hint.primary !== "local_qr_path") process.exit(1); if (!json.render_plan || json.render_plan.kind !== "auth_qr" || json.render_plan.proof_rule !== "This human action is not payment proof." || !json.render_plan.selected) process.exit(1); if (!json.render_plan.platforms.telegram.buttons.length) process.exit(1); if (action.qr_png_url || action.qr_image_url || action.preferred_qr_url) process.exit(1);})'
...
printf '%s' "$BUY_TELEGRAM_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const selected=json.render_plan && json.render_plan.selected; if (!selected || selected.platform !== "telegram" || selected.format !== "photo_text_inline_buttons") process.exit(1); if (!selected.media.length || selected.media[0].fallback_url !== "http://127.0.0.1/mock-qr.png") process.exit(1); if (!selected.links.some((link)=>link.url === "http://127.0.0.1/mock-pay")) process.exit(1); if (selected.buttons.map((button)=>button.intent).join(",") !== "refresh_payment_qr,check_payment_status") process.exit(1);})'
...
printf '%s' "$BUY_UNKNOWN_OUTPUT" | node -e 'let data="";process.stdin.on("data",c=>data+=c);process.stdin.on("end",()=>{const json=JSON.parse(data); const selected=json.render_plan && json.render_plan.selected; if (!selected || selected.platform !== "plain_chat" || selected.format !== "image_or_link_then_human_reply") process.exit(1); if (!selected.links.some((link)=>link.url === "http://127.0.0.1/mock-pay")) process.exit(1); if (!selected.text.includes("扫码")) process.exit(1);})'
