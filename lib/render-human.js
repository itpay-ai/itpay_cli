...
async function renderHumanAction(action, flags = {}) {
  ...
  if (mode === "file" || flags.qr_file || process.env.ITP_QR_FILE) {
    ...
    if (qrImageURL) {
      ...
      process.stderr.write(`${humanActionProviderLabel(action)} QR image: ${file}\n`);
      renderResult.rendered = true;
      ...
      if (localPath) {
        attachAgentLocalQR(action, localPath);
        annotateHumanActionPresentation(action, "");
        process.stderr.write(`${humanActionProviderLabel(action)} action QR image: ${localPath}\n`);
        renderResult.rendered = true;
  ...
  if (qrImageURL) {
    process.stderr.write(`${humanActionProviderLabel(action)} QR image URL: ${qrImageURL}\n`);
  ...
  if ((mode === "auto" || mode === "terminal") && shouldRenderTerminalQR(host)) {
    ...
    process.stderr.write(`\n${humanActionTitle(action)}\n`);
    if (action.description) process.stderr.write(`${action.description}\n`);
    process.stderr.write(`${qr}\n`);
    process.stderr.write(`${humanActionProviderLabel(action)} action URL: ${action.url}\n`);
  ...
  process.stderr.write(`${action.fallback_text || `Open ${humanActionProviderLabel(action)} URL: ${action.url}`}\n`);
